...
import { isLocalFunAsrConfigured, LocalFunAsrProvider } from './local-funasr-provider.mjs';
...
export function listTranscriptionProviders() {
  return [
    { id: 'fake', label: 'Fake ASR（演示链路）', configured: true },
    {
      id: 'funasr-local',
      label: '本地 FunASR（免费）',
      configured: isLocalFunAsrConfigured(),
    },
    {
      id: 'xunfei-ifasr-llm',
      label: '讯飞录音文件转写大模型',
...
export function getTranscriptionProvider(name = 'fake', options = {}) {
  ...
  if (name === 'funasr-local') return new LocalFunAsrProvider(options);
  ...
  throw new Error(`转写服务 ${name} 尚未接入。当前可用：funasr-local、fake、xunfei-ifasr-llm、xunfei-lfasr。`);
}
