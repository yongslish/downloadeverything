...
//   3. video-asr   – for video notes only, and only when the native post text
//                     is too thin to stand on its own. Extracts audio with
//                     ffmpeg and transcribes it with LocalFunAsrProvider.
//
...
const DEFAULT_MIN_POST_TEXT_LENGTH = 30;
 ...
 * @param {number} [options.minPostTextLength=30] - Character-length threshold below which the
 *   native post text (title + description) is considered "too thin" and the video-asr stage is
 *   attempted.
...
export async function extractXiaohongshuNote({
  ...
  asrProvider,
  audioLanguage = 'zh',
  minPostTextLength = DEFAULT_MIN_POST_TEXT_LENGTH,
  ...
  const postTextIsThin = combinedPostText.length < minPostTextLength;
  ...
  // Stage 3: video ASR fallback — only for video notes, and only when the
  // native post text is too thin to be useful on its own.
  if (noteType === 'video' && postTextIsThin) {
