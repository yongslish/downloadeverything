...
test('extracts metadata, description and chapters from yt-dlp info.json', async () => {
...
test('always transcribes with local FunASR when local media is available', async () => {
...
test('still attempts ASR even when the yt-dlp metadata call fails entirely', async () => {
  const fixtureDir = await createFixtureDir();
  const mediaPath = path.join(fixtureDir, 'video.mp4');
  await writeFile(mediaPath, 'fake media bytes');
  ...
  const metadataStage = doc.extraction.stagesAttempted.find((entry) => entry.stage === 'metadata');
  assert.equal(metadataStage.status, 'error');
  assert.ok(doc.extraction.stagesUsed.includes('asr'));
});
